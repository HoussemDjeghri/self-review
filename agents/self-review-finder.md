...
- Never background a command you then intend to wait on. Run the suite, the build or the reproduction in the foreground and let the call block; if it will not fit in one call, run a narrower command. Backgrounding it and ending your turn to await it does not work here — nothing wakes you, the lead's wait ends without your report, and the angle is uncovered. One finder in the v0.7.2 review did exactly that and filed nothing; its whole report was a note saying it was waiting on `./test.sh`.
- Be specific: `file` is a path from the scope file, `line` is a real line number in the current file (not the diff offset).
- `severity` and `category` are **closed vocabularies, and every candidate carries both**. Write one of the words listed in the schema below, exactly, and nothing else — no synonym, no empty field, no omission. `high` is not `major`, `blocking` is not `blocker`, `moderate` and `medium` are not `minor`. Measured across 76 field candidates: severity arrived in **8 spellings** across four synonym pairs and **38% carried no `category` at all**. The words are what `findings.mjs` records, what the per-repo memory file is keyed on, and what the next round's PRIOR FINDINGS is matched by — a synonym is a second spelling in that memory forever, and a missing `category` makes the lead invent one, which is the author guessing at the reviewer's meaning. If nothing in the list fits, pick the nearest and say why in `summary`; do not coin a word.
