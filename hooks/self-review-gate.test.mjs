...
const MARKER_CMD = `${CONVERGED_Q} "rounds=1 fixed=0 dismissed=0 open=0"`;
const MARKER_OUT = "SELF-REVIEW CONVERGED — rounds=1 fixed=0 dismissed=0 open=0";
...
test("a marker is found after a data heredoc whose prose holds an apostrophe", () => {
  ...
  assert.ok(!blocks(run(turn(...write(`${PROJECT}/src/a.ts`), ...bash(ledgerThenMarker, MARKER_OUT)))));
});
...
test("the marker is recognised behind an assignment, time, sudo, or bash -c", () => {
  ...
  for (const form of forms) assert.ok(!blocks(run(turn(...write(`${PROJECT}/src/a.ts`), ...bash(form, MARKER_OUT)))), form);
});
...
test("a marker after the notification releases; any agent type counts as pending", () => {
  const clean = run([human("go"), ...write(`${PROJECT}/a.js`), ...launch("ag1", "general-purpose"), said("waiting"), notification("ag1"), ...marker(), said("done")]);
...
test("a CONVERGED.json write in a scratch dir after the last change releases the turn and is logged", () => {
  ...
  const r = run(turn(...write(`${PROJECT}/src/a.ts`), ...writeMarker({ outcome: "converged", rounds: 2, fixed: 1, dismissed: 0, open: 0 })), { env: { SELF_REVIEW_LOG_DIR: logDir } });
  assert.equal(r.stdout, "");
...
test("a rejected marker corrected in the same turn releases; the correction is what counts", () => {
  const entries = turn(...write(`${PROJECT}/src/a.ts`), ...writeMarker({ outcome: "converged" }), ...writeMarker());
...
test("the write marker is logged once even when the gate runs again in the same turn", () => {
  const logDir = mkdtempSync(path.join(tmpdir(), "srlog-"));
  const entries = turn(...write(`${PROJECT}/src/a.ts`), ...writeMarker());
...
test("a CONVERGED.json written inside the project, not a scratch dir, is not a marker", () => {
  ...
  assert.equal(run(turn(...write(`${PROJECT}/src/a.ts`), ...writeMarker(CONVERGED_RECORD, mkdtempSync(path.join(tmpdir(), "sr-"))))).stdout, "");
});
...
test("the marker command the gate prints is runnable where the install path has a space", () => {
  ...
  try {
    ...
    const cleared = run(turn(...write(`${PROJECT}/src/a.ts`), ...bash(`${printed} "rounds=1"`, MARKER_OUT)), { gate });
...
test("the write marker counts when the Write overwrote an existing file (the harness's 'updated successfully' wording)", () => {
  ...
  assert.equal(run(turn(...write(`${PROJECT}/src/a.ts`), ...overwrite)).stdout, "");
});
